...
Linear Regression analyzing the relationship between Partisan Affiliation and Accuracy of social media posts.

```{r}
parties <- parties |>
  mutate(PartyBinary = if_else(Party == "Republican", 1, 0))
model <- lm(accuracy_rating ~ PartyBinary, data = parties)
summary(model)
```
The results of the linear regression indicate the average accuracy rating for the Democratic party is 3.023, as this is the Intercept, and the Democratic party was assigned the value '0'. The PartyBinary coefficient of 0.04421 indicates 

To answer my research question, I was incorrect and my hypothesis proven wrong. On average, the media produced by the Republican Party received a slightly higher accuracy rating on average than the media produced by the Democratic Party.
